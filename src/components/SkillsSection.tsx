...
const SkillsSection: React.FC<SkillsSectionProps> = ({ scrollY }) => {
  ...
  const getLevelIcon = (proficiency: number) => {
    ...
    switch (level) {
      ...
      case "Advanced":
        return <Award className="w-4 h-4 text-red-400 fill-current" />;
      case "Intermediate":
  ...
  return (
    <section
      id="skills"
      className="py-20 relative overflow-hidden bg-charcoal/50"
    >
                    ...
                    {categorySkills.map((skill, skillIndex) => (
                      ...
                      >
                        <div className="flex items-center justify-between p-3 rounded-lg bg-charcoal/30 hover:bg-charcoal/50 transition-all duration-300 border border-transparent hover:border-neon/20">
                          ...
                          <motion.div
                            className={`flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold border ${getLevelColor(
                              skill.proficiency
                            )}`}
